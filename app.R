...
mytheme <- create_theme(
  theme = "flatly",
  adminlte_color(light_blue = "#1B1A17"),
  adminlte_sidebar(
    width = "320",
    dark_bg = "#1B1A17",
...
ui <- tagList(
  dashboardPage(
    ...
    dashboardBody(
      ...
      use_theme(mytheme),
      tags$style(
        ".small-box.bg-teal { background-color: #F0A500 !important; color: #000000 !important; height: 140px}"
      ),
      tags$style(
        ".small-box.bg-yellow { background-color: #F0A500 !important; color: #000000 !important; height:140px}"
      ),
      tags$style(
        ".small-box.bg-red { background-color: #F0A500 !important; color: #000000 !important; height:140px}"
      ),
      tabItems(
        tabItem(
          tabName = "overview",
        ...
        tabItem(
          ...
          fluidRow(
            valueBoxOutput("plat_box"),
            valueBoxOutput("most_recent_plat"),
  ...
  tags$footer(
    HTML(
      "Designed and developed by <a href=mailto:terje.m.espedal@gmail.com>Terje Espedal</a>"
    ),
    tags$style(
      "footer
        {
...
server <- function(session, input, output) {
    ...
    reactive(
      data() %>% unnest(
        ...
        left_join(
          ...
          by = c("platforms_platform_family" = "id")
        ) %>%
        rename(name = name.x, platforms_family_name = name.y) %>%
        mutate(year = year(
          as.POSIXct(release_dates_date, origin = "1970-01-01")
        )) %>%
        mutate(
          platform_family_name = ifelse(
            platforms_name == "PC (Microsoft Windows)",
            "PC",
            platforms_family_name
          )
        ) %>%
        mutate(platform_family_name = factor(
          replace_na(platform_family_name, "Other")
        )) %>%
        mutate(
          platforms_name = ifelse(
            platforms_name == "PC (Microsoft Windows)",
            "PC",
            platforms_name
          )
  ...
  output$plot <-
    ...
    renderPlotly({
      ...
      platforms() %>%
        ...
        facet_grid(platform_family_name ~ .,
                   scales = "free_y",
                   switch = "y") +
        ...
        #theme_minimal() +
        labs(
          title = "Number of games released per platform over time",
          subtitle = "The death of Sega",
          y = "Count of games",
          x = ""
        ) +
        theme(
  ...
  output$genre_plot <-
    renderPlotly({
      data() %>%
        ...
        ggplot(aes(
          y = reorder(factor(genres_name), genre_count),
          x = genre_count,
          fill = genres_name
        )) +
    ...
    renderPlotly({
      ...
      themes <- data() %>%
        ...
        mutate(
          themes_name = replace(
            themes_name,
            themes_name == "4X (explore, expand, exploit, and exterminate)",
            "4X"
          )
        ) %>%
        summarise(theme_count = n()) %>%
        ggplot(aes(
          y = reorder(themes_name, theme_count),
          x = theme_count,
          fill = themes_name
        )) +
        geom_col(show.legend = FALSE) +
  ...
  output$most_recent_plat <- renderValueBox({
    ...
    valueBox(
      value = HTML(
        paste(
          "Year:",
          latest_platform %>% pull(min_year),
          "<br>Platform: ",
          latest_platform %>% pull(platforms_name)
        )
      ),
  ...
  output$most_pop_plat_fam <- renderValueBox({
    platform_count <-
      platforms() %>% group_by(platforms_name) %>% count(platforms_name) %>%
      arrange(desc(n)) %>% head(1)
    valueBox(
      value = HTML(
        paste0(
          "Count: ",
          platform_count %>% pull(n),
          "<br>Platform: ",
          platform_count %>% pull(platforms_name)
        )
      ),
  ...
  output$biggest_collection <- renderValueBox({
    ...
    valueBox(
      value = HTML(
        paste(
          collection_count %>% pull(collection),
          "<br>Count:",
          collection_count
          %>% pull(col_count)
        )
  ...
  output$most_used_keyword <- renderValueBox({
    keywords <-
      data() %>% unnest(keywords, names_repair = "unique", names_sep = "_") %>%
  ...
  output$box1 <- renderValueBox({
    games <-
      games_with_reviews  %>% filter(first_release_year == max_year) %>%
      arrange(
        desc(aggregated_rating),
        desc(aggregated_rating_count),
        desc(follows),
        desc(hypes),
        desc(total_rating_count)
      ) %>% head(1)
    valueBox(
